...
from app.clients.chan_api import ChanAPIClient
from app.models import Board, CatalogThread, Thread, ThreadPost, image_url, thumbnail_url
from app.text import extract_all_quotes, extract_quotes, html_to_text, strip_header_quotes, text_to_html
...
def _build_post_payload(
    ...
    is_video = post.ext == ".webm"
    country = post.country
    country_name = post.country_name
    country_code = country.lower() if country else None
    country_flag_url = (
        f"https://s.4cdn.org/image/country/{country_code}.gif" if country_code else None
    )
    return {
        "no": post.no,
        ...
        "image_view_href": (
            f"/board/{board}/thread/{thread_id}/post/{post.no}" if full_image_url else None
        ),
        "image_full_href": (
            f"/board/{board}/thread/{thread_id}/post/{post.no}/image" if full_image_url else None
        ),
    }


async def _load_thread_posts(board: str, thread_id: int) -> list[dict]:
    payload = await client.fetch_json(f"/{board}/thread/{thread_id}.json", ttl_seconds=10)
...
async def home(request: Request, selected: Optional[str] = None):
    try:
    ...
    selected_board = next((board for board in boards if board.board == selected), boards[0])
    board_items = []
    for board in boards:
        board_items.append(
            {
...
async def catalog(request: Request, board: str, selected: Optional[int] = None):
    try:
        payload = await client.fetch_json(f"/{board}/catalog.json", ttl_seconds=30)
    except HTTPStatusError as exc:
        status_code = exc.response.status_code
        message = "Board not found." if status_code == 404 else "Unable to load catalog right now."
        return templates.TemplateResponse(
            "catalog.html",
    ...
    for page in payload:
        for item in page.get("threads", []):
            ...
            country_code = thread.country.lower() if thread.country else None
            country_flag_url = (
                f"https://s.4cdn.org/image/country/{country_code}.gif"
                if country_code
                else None
            )
            threads.append(
...
async def thread(request: Request, board: str, thread_id: int, selected: Optional[int] = None):
    try:
        posts = await _load_thread_posts(board, thread_id)
    except HTTPStatusError as exc:
        status_code = exc.response.status_code
        message = "Thread not found." if status_code == 404 else "Unable to load thread right now."
        return templates.TemplateResponse(
            "thread.html",
            {
                "request": request,
...
@app.get("/board/{board}/thread/{thread_id}/post/{post_id}", response_class=HTMLResponse)
async def post_view(request: Request, board: str, thread_id: int, post_id: int):
    try:
        posts = await _load_thread_posts(board, thread_id)
    except HTTPStatusError as exc:
        status_code = exc.response.status_code
...
@app.get("/board/{board}/thread/{thread_id}/post/{post_id}/image", response_class=HTMLResponse)
async def post_image(request: Request, board: str, thread_id: int, post_id: int):
    try:
        posts = await _load_thread_posts(board, thread_id)
    except HTTPStatusError as exc:
        status_code = exc.response.status_code
